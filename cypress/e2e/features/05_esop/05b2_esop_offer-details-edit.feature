...
Feature: ESOP Offer - Set up plan enhancements
  ...
  @ESOPRegression @PLX-3527 
  Scenario Outline: When offer status Open, issuer user with the esop role should be able perform functionality granted for Open status - Auto
    ...
    And in 'Edit Offer' pop-up, <Vesting Schedule> radio is selected for 'Vesting Schedule'
    And in 'Edit Offer' pop-up, '<Frequency>' is selected for Frequency
    And in 'Edit Offer' pop-up, '<Vesting Amount>' is entered into Vesting Amount(%)
    And in 'Edit Offer' pop-up, '<Rounding>' is selected for Rounding
    And in 'Edit Offer' pop-up, 'Issue Price not Applicable' checkbox is disabled
    # And in 'Edit Offer' pop-up, '<Issue Price>' is entered into Issue Price
    And in 'Edit Offer' pop-up, 'Exercise Price not Applicable' checkbox is disabled
    ...
    And in 'ESOP' page, 'Details' tab, '<Description>' is displayed for Description
    ...
    And in 'ESOP' page, 'Details' tab, '<Vesting Schedule>' is displayed for Type
    And in 'ESOP' page, 'Details' tab, '<Frequency>' is displayed for Frequency
    And in 'ESOP' page, 'Details' tab, '<Vesting Amount>' is displayed for Vesting Amount

    @TestCaseKey=PLX-T324 @DEV @TEST @run
    Examples:
      ...
      | Owner  | IC Trading Co | CYPRESS-ESOP 2023Q1#3 | CYPRESS-ESOP Offer 2 Closed Auto | ESOP4 Offer 2 for closing auto | ESOP4       | [Today]    | unchecked                 | [Today] + 2 | unchecked                  | Auto             | Half Yearly | 33%            | Round Down | 10.5        | unchecked                  | 5.5            | unchecked                     | [Tomorrow] | Yes             |

  @ESOPRegression @PLX-3527 
  Scenario Outline: When offer status Open, issuer user with the esop role should be able perform functionality granted for Open status - Manual
    ...
    @TestCaseKey=PLX-T325 @DEV @TEST @run
    Examples:
      ...
      | Owner  | IC Trading Co | CYPRESS-ESOP 2023Q1#3 | CYPRESS-ESOP Offer 1 Closed Manual | ESOP8 Offer 1 closing manual | ESOP8       | [Today] + 1 | unchecked                 | [Today] + 90 | unchecked                  | Manual           | [Today] + 1  | 40%            | Nearest  | -           | checked                    | -              | checked                       | [Today] + 90 | No              |